...
class Table {
    ...
    createTable() {
        ...
        this.goalScale.domain([d3.min(this.teamData.map(function(item) { return item.value['Goals Made'] })), d3.max(this.teamData.map(function(item) { return item.value['Goals Made'] }))])
        ...
        d3.select('#goalHeader').append('svg').attr('width', this.cell.width * 2)
    ...
    updateTable() {
        ...
        let gameScale = this.gameScale.domain([d3.min(this.tableElements.map(function(item) { return item.value.Wins})), d3.max(this.tableElements.map(function(item) { return item.value.TotalGames}))])
        .range([0, this.cell.width]);
        let aggregateColorScale = this.aggregateColorScale.domain([d3.min(this.tableElements.map(function(item) { return item.value.Wins})), d3.max(this.tableElements.map(function(item) { return item.value.TotalGames}))])
        .range(['#ece2f0', '#016450']);
        ...
        let goalScale = this.goalScale.domain([d3.min(this.tableElements.map(function(item) { return item.value['Goals Made'] })), d3.max(this.tableElements.map(function(item) { return item.value['Goals Made'] }))])
        .range([5, this.cell.width * 2 - 10]);
        let goalColorScale = this.goalColorScale.domain([d3.min(this.tableElements.map(function(item) { return item.value['Delta Goals'] })), 0, d3.max(this.tableElements.map(function(item) { return item.value['Delta Goals'] }))])
    ...
    updateList(i) {
       ...
       for (var I = 0; I < Games.length; I++) {
       		this.tableElements.splice(i + I + 1, 0, Games[I])  }
    ...
    sortTable(header) {
        ...
        for (var i=0; i < types.length; i++){
        	if(types[i] == 'game') { arr.push(i) }}

        for(i=arr.length - 1; i>=0; i--){
		this.tableElements.splice(arr[i], 1) }

		// Sort array
		var Header = this.tableHeaders[header];
